...
class SessionState:
            ...
            self.sessions[session_id] = {
                'threads': {
                    'default': {
                        ...
                        'patient_id': None,
                        'visit_id': None,
                        'thread_type': 'scenario',  # NEW: 'scenario' or 'visit'
                        'is_saved_visit': False     # NEW: Track if this is a saved visit
                    }
                },
    ...
    def create_thread(self, session_id, thread_name, thread_type='scenario'):
        ...
        # Add type prefix to name for clarity
    ...
    def get_visit_comparison_data(self, session_id, visit_ids=None):
        ...
        # If specific visits not requested, compare recent visits
        if not visit_ids:
...
# Helper function to append results to Excel
...
def enhanced_chat_with_medbot(question, chat_history, session_id):
    ...
    history_keywords = [# Explicit requests
...
"""
    ...
    # ===== 2. VISIT COMPARISON PROMPT (KEEPS PAST JSONs) =====
...
"""
    ...
    # ===== 5. DEFAULT PATIENT-SPECIFIC PROMPT (CURRENT JSON + SUMMARY) =====
    else:
        prompt = f"""
...
def handle_json_text_input(json_text, chat_history, session_id):
    ...
    try:
        ...
        # Save visit to database if patient is selected
...
def select_patient_with_persistence(patient_input, session_id):
    ...
    # Search for existing patients in DATABASE (not just current session)
    results = session_state.memory_db.search_patients(patient_input)
    
    if results:
        # Existing patient found in database
        patient_id, name, last_visit = results[0]
        
        # Load patient into current session
        visit_count = session_state.set_current_patient(session_id, patient_id, name)
        
        # Get patient's full history from database
        ...
        if history:
            history_summary = f"\n\n📋 Recent Visits:\n"
            for i, visit in enumerate(history[:3]):  # Show last 3 visits
                visit_date = datetime.fromisoformat(visit['timestamp']).strftime('%Y-%m-%d %H:%M')
                findings_count = len(visit['findings']) if visit['findings'] else 0
        ...
        # New patient - create in database
...
def create_gradio_interface():
    with gr.Blocks(title="DentalMed AI Assistant") as demo:
        ...
        def trigger_visit_comparison(session_id):
            """Trigger visit comparison analysis"""
            comparison_response = enhanced_chat_with_medbot(
                "Compare visits and show progression analysis", 
                [], 
                session_id
            )
            return comparison_response[1]  # Return updated chat history
